...
public class TransactionControllerTest {
	...
	public void retrieveTransactionsTest() throws Exception {
		when(transactionService.retrieveTransactions())
				.thenReturn(Arrays.asList(new Transaction(1L, "type", 100.00, new Date(), "note")));
	...
	public void retrieveTransactionTest() throws Exception {
		when(transactionService.findTransaction(1L))
				.thenReturn(new Transaction(1L, "type", 100.00, new Date(), "note"));
	...
	public void updateTransactionTest() throws Exception {
		...
		when(transactionService.updateTransaction(any(), any()))
				.thenReturn(new Transaction(1L, "typeUpdated", 100.00, new Date(), "note"));
		mockMvc.perform(
	...
	public void findInvalidTransactionTest() throws Exception {
		when(transactionService.findTransaction(1L)).thenThrow(new TransactionNotFoundException("Transaction not found"));
		mockMvc.perform(MockMvcRequestBuilders.get("/transactions/1")).andExpect(MockMvcResultMatchers.status().isNotFound());
	}
}
