...
public class TransactionControllerTest {
    ...
    public void retrieveTransactionsTest() throws Exception {
        when(transactionService.retrieveTransactions("id", 0, 20)).thenReturn(new PageImpl<>(Arrays.asList(new Transaction("id", "type", 100.00,
        ...
        mockMvc.perform(MockMvcRequestBuilders.get("/transactions")).andExpect(MockMvcResultMatchers.status().isOk())
                ...
                .andExpect(MockMvcResultMatchers.jsonPath("@.content.[0].amount").value(100.00))
                .andExpect(MockMvcResultMatchers.jsonPath("@.content.[0].date")
                        .value(new SimpleDateFormat("yyyy-MM-dd").format(new Date())))
                .andExpect(MockMvcResultMatchers.jsonPath("@.content.[0].note").value("note"))
                .andExpect(MockMvcResultMatchers.jsonPath("@.content.[0].category").value("category"));
    }
